...
本仓库也包含了我自己写的**实验报告**及所用学长的参考报告和资料、AI提示词、2023级课程要求等，这些都放在了```\完整实验报告及资料```目录下。

同时，我也在release里添加了一个压缩包，名为```BIT_CEC_DC_Report_and_Code_YanZiyi.zip```，里面是我的实验报告以及我修改和添加的5个代码文件，如果你对整个项目的运行流程已经熟悉了，下载这个压缩包应该就够了。
...
如果要复现的话，你可以直接克隆这个仓库（比较省事，克隆下来把整个文件夹放到无中文路径下就可以直接编译了）；也可以在原有gitee项目的基础上替换上面**5个**文件。

这5个文件还有vcd波形文件在```\完整实验报告及资料\课设数字系统_源代码```路径下单独存放，可以自行替换。
...
直接克隆这个Github仓库，放到无中文路径的文件夹下。

TASK2是根据gitee初始项目的 timer_int 计时器中断例程改写的，所以cmd进入```\tests\example\timer_int```文件夹，依次执行
