...
TASK1老师会在视频里带着做，很容易完成。所以我设置仓库呈现的默认分支是TASK2的代码，下面说的主要也是 TASK2 部分的代码。
...
增加了TASK1和TASK2的bin文件和波形，可以直接验证题目要求功能是否实现。
...
TASK2是根据gitee初始项目的 timer_int 计时器中断例程改写的，所以cmd进入```tests\example\timer_int```文件夹，依次执行执行
...
命令行上会出现大的 **PASS**，并会在下面一行显示最终的sum_result是多少，比如
